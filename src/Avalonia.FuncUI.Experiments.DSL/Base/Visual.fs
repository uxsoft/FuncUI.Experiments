...
type VisualBuilder<'t when 't :> Visual>() =
    ...
    member _.opacity<'t>(x: DSLElement<'t>, value: float) =
        x @@ [  AttrBuilder<'t>.CreateProperty<float>(Visual.OpacityProperty, value, ValueNone) ]
    ...
    member _.renderTransform<'t>(x: DSLElement<'t>, transform: ITransform) =
        x @@ [  AttrBuilder<'t>.CreateProperty<_>(Visual.RenderTransformProperty, transform, ValueNone) ]
